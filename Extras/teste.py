...
for path in paths:
    ...
    name = os.path.basename(path).replace('Defsaida30-','')  # ex: "FCFS", "Prioridade", "SRTN"
    with open(path) as f:
...
max1    = max(values1)
ymax1   = math.ceil(max1 / y_step) * y_step  # arredonda pra cima ao múltiplo de y_step

plt.figure(figsize=(6,4))
plt.bar(algs, values1)
plt.title('Cumprimento de Deadlines (entrada‑inesperado) - Máquina A')
...
max2    = max(values2)
ymax2   = math.ceil(max2 / y_step) * y_step

plt.figure(figsize=(6,4))
plt.bar(algs, values2)
plt.title('Número de Preempções (entrada‑inesperado) - Máquina A')
