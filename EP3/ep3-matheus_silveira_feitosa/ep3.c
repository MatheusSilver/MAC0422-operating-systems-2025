...
void init_next_fit_pointer(FILE *memoryFile) {
    ...
    while (get_next_pgm_status(memoryFile, &curStatus)) {
        ...
        if (curStatus == EMPTY) {
            ...
            if (!inEmptySequence) {
                lastEmptyStart = curPos;  
                /* Usado para verificar se uma nova sequencia começou, basicamente serve pra não reinicar o lastEmpty o tempo todo. */
    ...
    /* Se a última posição lida for parte de uma sequência vazia, então então assumimos que                   */
    /* A escrita no PGM parou nesta última posição de leitura da memória.                                     */
    /* Caso contrário, voltamos para o primeiro EMPTY encontrado assumindo que o ponteiro retornou pro começo */
    /* E parou em firstEmptyPos após circular a memória.                                                      */
...
void move_back_file_descriptor(FILE* memoryFile, int targetPos, int currentPos){
    ...
    /* Pelo enunciado, não poderia ser nenhuma outra coisa que não fosse um enter.   */
...
/* Eu literalmente fiz a worst fit apenas copiando e colando a best_fit */
/* Na pratica, teria feito tudo em uma função só com um parâmetro booleano, mas né? */
