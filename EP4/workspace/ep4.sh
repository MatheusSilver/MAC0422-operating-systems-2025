...
# >>>>>>>>>>>>>>>>>>>>>>>>> VERIFICAÇÃO INICIAL <<<<<<<<<<<<<<<<<<<<<<<<< #
if [[ $# -lt 2 ]]; then
    echo "Uso: $0 <quantidade de clientes> [<tamanho do arquivo 1> <tamanho do arquivo 2> ... <tamanho do arquivo n>]"
    exit 1
fi
...
# Usado apenas se alguma coisa der MUITO errado, ai forçamos a morte dos clientes abertos.
# Muito errado pois esperamos que eles se fechem sozinhos.
